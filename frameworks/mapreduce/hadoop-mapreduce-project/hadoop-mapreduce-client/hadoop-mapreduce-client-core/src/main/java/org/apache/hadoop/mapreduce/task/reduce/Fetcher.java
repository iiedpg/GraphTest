...
class Fetcher<K,V> extends Thread {
  ...
  private Record waitRecord = TimeRecorder.getRecord("copy_wait_merge");
  private Record waitMapRecord = TimeRecorder.getRecord("copy_wait_map");
  private Record copyRecord = TimeRecorder.getRecord("copy_from_map");
  private Record ioRecord = TimeRecorder.getRecord("copy_from_map_io");
    ...
    try {     	
      ...
      while (!stopped && !Thread.currentThread().isInterrupted()) {
        ...
        try {
          ...
          waitRecord.timerStart();
          merger.waitForResource();
          waitRecord.timerEnd();

          // Get a host to shuffle from
          waitMapRecord.timerStart();
          host = scheduler.getHost();
          waitMapRecord.timerEnd();
          metrics.threadBusy();

          // Shuffle
          copyRecord.timerStart();
          copyFromHost(host);
          copyRecord.timerEnd();
