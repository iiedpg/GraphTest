...
import org.apache.spark.gaoyun.TimeRecorder._
...
class DAGScheduler(
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        //var info = new StringBuffer("====dep====")
        //info.append(rdd.id).append(',').append(rdd.name).append(',').append(rdd.origin)
        ...
        if (getCacheLocs(rdd).contains(Nil)) {
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_,_] =>
                //info.append('@').append(dep.rdd.id).append(',').append(shufDep.shuffleId)
                val mapStage = getShuffleMapStage(shufDep, stage.jobId)
				//logInfo("create mapStage " + mapStage.id)
                if (!mapStage.isAvailable) {
                  missing += mapStage
                }
              case narrowDep: NarrowDependency[_] =>
                //info.append('@').append(dep.rdd.id)
                visit(narrowDep.rdd)
            }
          }
        }
    ...
    //logInfo("====stage_to_rdd====" + stage.id + "," + stage.rdd.id)
