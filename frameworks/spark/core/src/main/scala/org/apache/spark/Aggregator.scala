...
case class Aggregator[K, V, C] (
  ...
  def combineValuesByKey(iter: Iterator[_ <: Product2[K, V]],
	...
	var rSortTime = TimeRecorder.getRecord(context.partitionId + "," + 0 + "," + "map_side_shu_" + System.currentTimeMillis())
  ...
  def combineCombinersByKey(iter: Iterator[(K, C)], context: TaskContext) : Iterator[(K, C)] = {
    var rSortTime = TimeRecorder.getRecord(context.partitionId + "," + 0 + "," + "red_side_shu_" + System.currentTimeMillis())
	rSortTime.timerStart()
    if (!externalSorting) {
      val combiners = new AppendOnlyMap[K,C]
