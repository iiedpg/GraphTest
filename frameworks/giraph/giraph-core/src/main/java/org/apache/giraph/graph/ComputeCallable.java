  ...
  private PartitionStats computePartition(Partition<I, V, E, M> partition)
    ...
    synchronized (partition) {
      ...
      for (Vertex<I, V, E, M> vertex : partition) {
        ...
        //GraphTaskManager.getIteratorRecord.multiContinue();
        Iterable<M> messages = messageStore.getVertexMessages(vertex.getId());
        //GraphTaskManager.getIteratorRecord.multiPause();
        //long end = System.nanoTime();        
        
        //LOG.info("message_iterator_for " + vertex.getId() + " = " + (end - start)/1e6 + " ms ");
        ...
        if (!vertex.isHalted()) {
          //GraphTaskManager.iterAllRecord.multiContinue();
        
          context.progress();
          TimerContext computeOneTimerContext = computeOneTimer.time();
          try {
        	
        	//GraphTaskManager.computerAllRecord.multiContinue();
            vertex.compute(messages);
            //GraphTaskManager.computerAllRecord.multiPause();
          ...
          //GraphTaskManager.iterAllRecord.multiPause();
        }
        if (vertex.isHalted()) {
