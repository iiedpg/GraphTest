...
abstract class RDD[T: ClassTag](
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    if (storageLevel != StorageLevel.NONE) {
	  var iterRec = TimeRecorder.getRecord(split.index + "," + id + "," + "load_cache")
	  iterRec.timerStart()
      var r = SparkEnv.get.cacheManager.getOrCompute(this, split, context, storageLevel)
	  iterRec.timerEnd()
	  r
    } else {
	  var iterRec = TimeRecorder.getRecord(split.index + "," + id + "," + "load_computer")
	  iterRec.timerStart()
      var r = computeOrReadCheckpoint(split, context)
	  iterRec.timerEnd()
	  r
    }
  }
  ...
  def mapPartitionsWithContext[U: ClassTag](
    ...
    val func = (context: TaskContext, index: Int, iter: Iterator[T]) => {logInfo("#### computer " + id + "_" + index);f(context, iter)}
