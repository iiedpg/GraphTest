...
#include <graphlab/rpc/gaoyun.hpp>
...
namespace graphlab {
  ...
  private:
    ...
    void run_synchronous(MemberFunction member_fun) {
	  ...
	  //add_by_gy all context
	  TimerContext& all_context = get_timer_context("all");
	  all_context.multiContinue();
	  ...
	  //add_by_gy : all_context
	  all_context.multiPause();
  ...
  synchronous_engine<VertexProgram>::start() {
    ...
    // Program Main loop ====================================================
	// add_by_gy : time statistic
	TimerContext& all_context = get_timer_context("all");
	TimerContext& mess_count_context = get_timer_context("mess_count");

    while(iteration_counter < max_iterations && !force_abort ) {
	  //add_by_gy : all_context
	  all_context.multiStart();
	  long last_mess_count = get_mess_count();
	  ...
	  //add_by_gy : all_context
	  all_context.multiEnd();
	  long now_mess_count = get_mess_count();
	  mess_count_context.pushValue(now_mess_count - last_mess_count);
    }
