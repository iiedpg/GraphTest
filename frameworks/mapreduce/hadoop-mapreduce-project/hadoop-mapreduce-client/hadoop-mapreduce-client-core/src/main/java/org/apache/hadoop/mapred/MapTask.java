...
public class MapTask extends Task {
  ...
  public void run(final JobConf job, final TaskUmbilicalProtocol umbilical)
    throws IOException, ClassNotFoundException, InterruptedException {
	  ...
	  Record rrecord = TimeRecorder.getRecord("running_time");
		Record srecord = TimeRecorder.getRecord("start_time");
		
		long start = System.currentTimeMillis();
		srecord.increValue(start);
		rrecord.timerStart();
		
		TimeRecorder.addMemoryRecord("yarn_start"); 
    ...
    TimeRecorder.addMemoryRecord("afterTaskStop");
    
	Record erecord = TimeRecorder.getRecord("end_time");
    
    rrecord.timerEnd();
    long end = System.currentTimeMillis();
    erecord.increValue(end);    
    
    LOG.info(TimeRecorder.dump());
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  static class NewTrackingRecordReader<K,V> 
    ...
    private Record inputRecord = TimeRecorder.getRecord("input_kvs");
    private Record inputBytesRecord = TimeRecorder.getRecord("input_bytes");
    private Record inputTimes = TimeRecorder.getRecord("input_time");
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
      ...
      if (result) {
        inputRecordCounter.increment(1);
        inputRecord.increValue(1);
      }
      fileInputByteCounter.increment(bytesInCurr - bytesInPrev);
      
      inputBytesRecord.increValue(bytesInCurr - bytesInPrev);
  ...
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    //add_by_gy
    private Record collectRecord;    
    private Record mergeRecord;
    
    private Record collectNumberRecord;
    private Record collectBytesRecord;
    ...
    public void init(MapOutputCollector.Context context
      ...
      //add_by_gy
      collectRecord = TimeRecorder.getRecord("collect");
      mergeRecord = TimeRecorder.getRecord("merge_spill");
      collectNumberRecord = TimeRecorder.getRecord("collect_number");
      collectBytesRecord = TimeRecorder.getRecord("collect_bytes");
    ...
    public synchronized void collect(K key, V value, final int partition
      ...
      try {
        ...
        collectNumberRecord.increValue(1);
        collectBytesRecord.increValue(distanceTo(keystart, valend, bufvoid));
